...
const Header: React.FC<HeaderProps> = ({ 
  user, role, messName, messId, selectedMonth, onMonthChange, onMenuToggle, onViewChange, hasActiveMess, t
}) => {
  ...
  return (
    <header className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-gray-100 dark:border-gray-800 px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between sticky top-0 z-30 w-full overflow-hidden transition-colors duration-300">
      ...
      >
        <button 
          ...
          className="lg:hidden p-1 sm:p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg text-gray-500 dark:text-gray-300 transition-colors shrink-0"
        >
        ...
        <div className="flex flex-col min-w-0 group-hover:translate-x-0.5 transition-transform">
          <div className="flex items-center gap-1 sm:gap-2 min-w-0">
            <span className="font-black text-[10px] sm:text-base truncate text-gray-900 dark:text-gray-100 leading-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{user.name}</span>
            {getRoleBadge()}
          </div>
            ...
            <div className="flex items-center bg-gray-50 dark:bg-gray-800/80 rounded-lg sm:rounded-xl border border-gray-200 dark:border-gray-700 p-0.5 sm:p-1">
              ...
              <div className="relative flex items-center">
                <select 
                  ...
                  className="appearance-none bg-transparent border-none outline-none text-[9px] sm:text-xs font-black focus:ring-0 cursor-pointer text-blue-600 dark:text-blue-400 pl-1.5 sm:pl-2 pr-5 sm:pr-6 py-1 sm:py-1.5"
                >
                  {years.map(y => (
                    <option key={y} value={y} className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white">{y}</option>
                  ))}
                </select>
                <ChevronDown size={10} className="absolute right-1.5 text-blue-600 dark:text-blue-500 pointer-events-none" />
              </div>
            </div>
          </div>
        </>
