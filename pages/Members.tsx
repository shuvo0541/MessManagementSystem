    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-bold uppercase text-[10px] mt-1">সদস্যদের প্রোফাইল ও টাইমলাইন ম্যানেজমেন্ট</p>
        </div>

        <div className="flex bg-gray-900 p-1.5 rounded-2xl border border-gray-800 self-start sm:self-auto">
           <button 
             onClick={() => setViewType('list')}
             className={`px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 transition-all ${viewType === 'list' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-gray-500 hover:text-gray-300'}`}
           >
              <List size={16}/> লিস্ট ভিউ
           </button>
           <button 
             onClick={() => setViewType('timeline')}
             className={`px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 transition-all ${viewType === 'timeline' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-gray-500 hover:text-gray-300'}`}
           >
              <Calendar size={16}/> টাইমলাইন
           </button>
        </div>
      </div>
        ...
        {viewType === 'list' ? (
          ...
          <div className="p-8 space-y-6">
             {filteredUsers.map(u => {
               ...
               return (
                 <div key={u.id} className={`p-6 rounded-3xl border ${isCurrent ? 'bg-blue-600/5 border-blue-500/20' : 'bg-gray-800/20 border-gray-800'} space-y-4`}>
                    <div className="flex justify-between items-center">
                       <span className="font-black text-white">{u.name}</span>
                       <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg ${isCurrent ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-500'}`}>{isCurrent ? 'Active Now' : 'Inactive'}</span>
                    </div>
                    <div className="relative h-2 bg-gray-800 rounded-full overflow-hidden">
                       <div className="absolute inset-y-0 left-0 bg-blue-500 rounded-full shadow-lg shadow-blue-500/40" style={{ width: isCurrent ? '70%' : '100%' }} />
                    </div>
