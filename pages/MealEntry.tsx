...
const MealEntry: React.FC<MealEntryProps> = ({ month, userId, isAdmin, db, updateDB, t, theme }) => {
    ...
    <div className="space-y-6 pb-10 animate-in fade-in duration-500 overflow-x-hidden px-1 sm:px-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-white dark:bg-gray-900 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm self-start sm:self-auto w-full sm:w-auto">
           ...
           <input 
              type="date" 
              className="bg-transparent border-none text-[13px] sm:text-sm font-black text-gray-900 dark:text-white outline-none cursor-pointer p-1 flex-1"
              value={selectedDate}
      ...
      <div className="bg-blue-600/10 dark:bg-blue-900/10 border border-blue-500/20 p-5 sm:p-7 rounded-2xl sm:rounded-[2.5rem] flex items-center justify-between shadow-lg">
      ...
      <div className="sm:hidden space-y-4">
        {mealData.length === 0 ? (
          ...
          mealData.map(m => {
            ...
            return (
              <div key={m.userId} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-5 rounded-2xl space-y-5 shadow-sm">
                <div className="flex justify-between items-center border-b border-gray-100 dark:border-gray-800 pb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-blue-600/10 dark:bg-blue-900/20 text-blue-600 dark:text-blue-500 rounded-lg flex items-center justify-center font-black text-sm">{m.userName[0]}</div>
                    <span className="font-black text-gray-900 dark:text-white text-sm truncate max-w-[150px]">{m.userName}</span>
                  </div>
                  <div className="bg-blue-600/10 dark:bg-blue-900/20 px-3 py-1 rounded-full border border-blue-500/20">
                    <span className="text-blue-600 dark:text-blue-400 font-black text-xs">{rowTotal.toFixed(1)}</span>
                  </div>
                </div>
                  ...
                  {[
                    ...
                    <div key={field.id} className="space-y-1.5">
                      ...
                      <input 
                        ...
                        className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl py-3 text-center text-sm font-black text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-600 outline-none disabled:opacity-30"
      ...
      <div className="hidden sm:block bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 overflow-hidden shadow-2xl">
        <div className="overflow-x-auto no-scrollbar">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/40 text-[10px] uppercase font-black text-gray-500 border-b border-gray-100 dark:border-gray-800">
                <th className="px-8 py-6">{t.members}</th>
                <th className="px-4 py-6 text-center">{t.breakfast}</th>
                <th className="px-4 py-6 text-center">{t.lunch}</th>
                <th className="px-4 py-6 text-center">{t.dinner}</th>
                <th className="px-4 py-6 text-center">{t.guest}</th>
                <th className="px-8 py-6 text-right bg-blue-600/5 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400">{t.total}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {mealData.length === 0 ? (
                ...
                mealData.map(m => {
                  const rowTotal = m.breakfast + m.lunch + m.dinner + m.guest;
                  return (
                    <tr key={m.userId} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          <UserIcon size={14} className="text-gray-400" />
                          <span className="font-black text-gray-900 dark:text-white text-sm">{m.userName}</span>
                        </div>
                      </td>
                      {['breakfast', 'lunch', 'dinner', 'guest'].map(field => (
                        <td key={field} className="px-4 py-6 text-center">
                          <input 
                            type="number" step="0.5" min="0"
                            className="w-16 mx-auto bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-center py-2.5 text-sm font-black text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-600 outline-none disabled:opacity-30 transition-all"
                      ...
                      <td className="px-8 py-6 text-right bg-blue-600/5 dark:bg-blue-900/10">
                        <span className="text-xl font-black text-blue-600 dark:text-blue-500">{rowTotal.toFixed(1)}</span>
                      </td>
                    </tr>
                  );
                })
